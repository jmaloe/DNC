...
class CRoles extends CUsuario{
 ...
 function hayRolesSinAsignarByUser(){
	$sql = 'SELECT r.idRol FROM roles r WHERE r.idRol NOT IN(select rdu.idRol from rol_del_usuario rdu, usuarios_sistema us where rdu.idUser=us.idUser and us.idUser='.$this->getIdUsuario().')';
 	$data = $this->query($sql); 	
 ...
 function hayRolesByUser(){
 	$sql = "SELECT r.idRol FROM roles r, rol_del_usuario ru, usuarios_sistema u Where r.idRol=ru.idRol And ru.idUser=u.idUser And u.idUser=".$this->getIdUsuario();
 	$data = $this->query($sql);
 	if($fila=mysqli_fetch_row($data))
 		return true;
 	return false;
 ...
 function getRolesSinAsignarByUser(){
	$sql = 'SELECT r.idRol, r.nombreRol FROM roles r WHERE r.idRol NOT IN(select rdu.idRol from rol_del_usuario rdu, usuarios_sistema us where rdu.idUser=us.idUser and us.idUser='.$this->getIdUsuario().')';
 	$data = $this->query($sql); 	
 	if($data)
...
/*pasar -1 como parámetro para asignar selected a rol_default*/
 function getRolesByUser($default_select){
 	$sql = "SELECT r.idRol, r.nombreRol from roles r, rol_del_usuario ru, usuarios_sistema u Where r.idRol=ru.idRol And ru.idUser=u.idUser And u.idUser=".$this->getIdUsuario()." ORDER BY r.idRol;";
 	$resultado = $this->query($sql);
 	if($resultado)
 	{
